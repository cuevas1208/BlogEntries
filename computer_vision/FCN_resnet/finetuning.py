...
parser.add_argument('--model_path', default='/tmp/checkpoints/resnet_v1_50.ckpt', type=str)
...
parser.add_argument('--output_checkpoint', default='/tmp/models/animals_coco_vgg16_2/resnet_model.ckpt', type=str)

VGG_MEAN = [123.68, 116.78, 103.94]

'''
# see list of variables saved in the checkpoint
with tf.Session() as sess:
    from tensorflow.contrib.framework.python.framework import checkpoint_utils
    var_list = checkpoint_utils.list_variables('/tmp/checkpoints/vgg_16.ckpt')
    for v in var_list:
        print(v)
...
def main(args):
    ...
    # Get the list of file names and corresponding list of labels for training at validation
    ...
    with graph.as_default():
        # Standard pre-processing for VGG on ImageNet taken from here:
        # https://github.com/tensorflow/models/blob/master/research/slim/preprocessing/vgg_preprocessing.py
        # Also see the VGG paper for more details: https://arxiv.org/pdf/1409.1556.pdf

        # Pre-processing (for both training and validation):
        ...
        resnet_v1_50 = tf.contrib.slim.nets.resnet_v1
        # inputs has shape [batch, 224, 224, 3]
        with slim.arg_scope(resnet_v1_50.resnet_arg_scope()):
            logits, _ = resnet_v1_50.resnet_v1_50(images, num_classes=num_classes, is_training=is_training)
            # logits need to be flatten to match our labels
        ...
        variables_to_restore = tf.contrib.framework.get_variables_to_restore(exclude=['resnet_v1_50/block4',
                                                                                      'resnet_v1_50/logits'])
        init_fn = tf.contrib.framework.assign_from_checkpoint_fn(model_path, variables_to_restore)

        # Initialization operation from scratch for the new "fc8" layers
        # `get_variables` will only return the variables whose name starts with the given pattern
        fc8_variables = tf.contrib.framework.get_variables('resnet_v1_50/block4')
        fc8_init = tf.variables_initializer(fc8_variables)

        logits_variables = tf.contrib.framework.get_variables('resnet_v1_50/logits')
        ...
        fc8_train_op = fc8_optimizer.minimize(loss, var_list=[fc8_variables, logits_variables])
    ...
    with tf.Session(graph=graph) as sess:
        ...
        sess.run([fc8_init, logits_init])  # initialize the new fc8 layer
